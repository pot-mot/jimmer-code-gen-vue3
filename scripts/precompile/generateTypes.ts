...
const {program, modelTypeFiles, contextTypeFiles, scriptTypeFiles} = buildProjectTsProgram();
...
const jsonSchemaGenerator = tjs.buildGenerator(jsonSchemaProgram, {
    required: true,
    strictNullChecks: true,
});
if (jsonSchemaGenerator === null) {
    throw new Error('json schema generator is null');
}
...
export type TypeMap = {
${typeDeclares
    .filter((it) => !it.hasGenericType)
    .map((it) => `    ${it.typeName}: ${it.typeName}`)
    .join('\n')}
}
