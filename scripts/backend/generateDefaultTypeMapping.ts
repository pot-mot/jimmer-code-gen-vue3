...
const arrayToListExp = <T>(array: T[], toString: (it: T) => string, indentTab: number) => {
    if (array.length === 0) {
        return 'emptyList()';
    } else {
...
const initJvmTypesStr = initJvmTypes.map((it) => {
    return `    JvmTypeInput(
        ...
        sqlMatchRules = ${arrayToListExp(
            it.sqlMatchRules,
            (rule) => `JvmTypeInput.TargetOf_sqlMatchRules(
    ...
    matchRegExp = "${escapeRegExp(rule.matchRegExp)}",${
        ...
        tsMatchRules = ${arrayToListExp(
            it.tsMatchRules,
            (rule) => `JvmTypeInput.TargetOf_tsMatchRules(
    id = UUID.fromString("${rule.id}"),
    matchRegExp = "${escapeRegExp(rule.matchRegExp)}",
)`,
            2,
...
const initSqlTypesStr = initSqlTypes.map((it) => {
    return `    SqlTypeInput(
        ...
        type = "${it.type}",
        dataSize = ${it.dataSize ?? 'null'},
        numericPrecision = ${it.numericPrecision ?? 'null'},
        defaultValue = ${it.defaultValue ? `"${it.defaultValue}"` : 'null'},
        jvmMatchRules = ${arrayToListExp(
            it.jvmMatchRules,
            (rule) => `SqlTypeInput.TargetOf_jvmMatchRules(
    id = UUID.fromString("${rule.id}"),
    ...
    matchRegExp = "${escapeRegExp(rule.matchRegExp)}",
        ...
        tsMatchRules = ${arrayToListExp(
            it.tsMatchRules,
            (rule) => `SqlTypeInput.TargetOf_tsMatchRules(
    id = UUID.fromString("${rule.id}"),
    matchRegExp = "${escapeRegExp(rule.matchRegExp)}",
)`,
            2,
...
const initTsTypesStr = initTsTypes.map((it) => {
    ...
    typeOnly = ${importItem.typeOnly}
        ...
        jvmMatchRules = ${arrayToListExp(
            it.jvmMatchRules,
            (rule) => `            TsTypeInput.TargetOf_jvmMatchRules(
    id = UUID.fromString("${rule.id}"),
    jvmSource = JvmLanguageOrAny.${rule.jvmSource},
    matchRegExp = "${escapeRegExp(rule.matchRegExp)}"
)`,
        ...
        sqlMatchRules = ${arrayToListExp(
            it.sqlMatchRules,
            (rule) => `            TsTypeInput.TargetOf_sqlMatchRules(
    id = UUID.fromString("${rule.id}"),
    databaseSource = DatabaseTypeOrAny.${rule.databaseSource},
    matchRegExp = "${escapeRegExp(rule.matchRegExp)}"
)`,
