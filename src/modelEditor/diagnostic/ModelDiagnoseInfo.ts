...
import {
    entityDiagnose,
    type EntityDiagnoseResult,
} from '@/modelEditor/diagnostic/entityDiagnose.ts';
import {
    enumerationDiagnose,
    type EnumerationDiagnoseResult,
} from '@/modelEditor/diagnostic/enumerationDiagnose.ts';
import {
    embeddableTypeDiagnose,
    type EmbeddableTypeDiagnose,
} from '@/modelEditor/diagnostic/embeddableTypeDiagnose.ts';
import {
...
import {
    type AssociationDiagnose,
    associationDiagnose,
} from '@/modelEditor/diagnostic/associationDiagnose.ts';
...
export const useModelDiagnoseInfo = (
    ...
    const setGroupDiagnose = (group: DeepReadonly<Group>) => {
        modelDiagnoseInfo.groupMap.set(
            group.id,
            groupDiagnose(group, contextData, inheritInfo, nameSets),
        );
    };
    ...
    const setEntityDiagnose = (entity: DeepReadonly<EntityWithProperties>) => {
        modelDiagnoseInfo.entityMap.set(
            entity.id,
            entityDiagnose(entity, contextData, inheritInfo, nameSets),
        );
    };
    ...
    const setMappedSuperClassDiagnose = (
        mappedSuperClass: DeepReadonly<MappedSuperClassWithProperties>,
    ) => {
        modelDiagnoseInfo.mappedSuperClassMap.set(
            mappedSuperClass.id,
            mappedSuperClassDiagnose(mappedSuperClass, contextData, inheritInfo, nameSets),
        );
    };
    ...
    const setEmbeddableTypeDiagnose = (
        embeddableType: DeepReadonly<EmbeddableTypeWithProperties>,
    ) => {
        modelDiagnoseInfo.embeddableTypeMap.set(
            embeddableType.id,
            embeddableTypeDiagnose(embeddableType, contextData, inheritInfo, nameSets),
        );
    };
    ...
    const setEnumerationDiagnose = (enumeration: DeepReadonly<Enumeration>) => {
        modelDiagnoseInfo.enumerationMap.set(
            enumeration.id,
            enumerationDiagnose(enumeration, contextData, inheritInfo, nameSets),
        );
    };
    ...
    const setAssociationDiagnose = (association: DeepReadonly<AssociationIdOnly>) => {
        modelDiagnoseInfo.associationMap.set(
            association.id,
            associationDiagnose(association, contextData, inheritInfo, nameSets),
        );
    };
    ...
    const addSameNameAssociation = (
        ...
        for (const {association} of contextData.associationMap.values()) {
            if ('name' in association && association.name === name)
        ...
        if (inheritItem !== undefined) {
            ...
            for (const childId of inheritItem.abstractChildIdSet) {
                if (!unvisitedIdSet.has(childId)) {
                    ...
                    if (child !== undefined)
                        collectMappedSuperClassNeedDiagnose(
                            child,
                            undefined,
                            subIdSets,
                            unvisitedIdSet,
            ...
            if (missingDependency.has(mappedSuperClassId)) {
                if (contextData.entityMap.has(missingId)) {
                    if (!unvisitedIdSet.has(missingId)) {
                        ...
                        if (child !== undefined)
                            collectEntityNeedDiagnose(child, undefined, subIdSets);
                    }
        ...
        for (const entity of contextData.entityMap.values()) {
            for (const property of entity.properties) {
                if (
                    ...
                    property.embeddableTypeId === embeddableTypeId
                ) {
                    subIdSets.entityIdSet.add(entity.id);
                    break;
                }
    ...
    const collectMappedSuperClassNeedDiagnose = (
        ...
        if (
            oldMappedSuperClass !== undefined &&
            oldMappedSuperClass.name !== mappedSuperClass.name
        ) {
    ...
    const collectAssociationNeedDiagnose = (
        ...
        for (const other of contextData.associationMap.values()) {
            if (other.association.referencedEntityId === association.referencedEntityId)
                subIdSets.associationIdSet.add(other.association.id);
        }
        for (const entity of contextData.entityMap.values()) {
            for (const property of entity.properties) {
    ...
    const syncGroup = (group: DeepReadonly<Group>, oldGroup?: DeepReadonly<Group>) => {
    ...
    const syncMappedSuperClass = (
        ...
        const subIdSets = collectMappedSuperClassNeedDiagnose(
            mappedSuperClass,
            oldMappedSuperClass,
        );
        syncDiagnoseByIdSets(subIdSets);
    };
    const removeMappedSuperClass = (
        mappedSuperClass: DeepReadonly<MappedSuperClassWithProperties>,
    ) => {
    ...
    history.eventBus.on('change', (data) => {
        ...
        if (data.key === 'group:add') {
            ...
            if (data.type === 'apply')
                syncMappedSuperClass(getMappedSuperClass(data.options.mappedSuperClass.id));
            else if (data.type === 'revert') removeMappedSuperClass(data.options.mappedSuperClass);
        } else if (data.key === 'embeddable-type:add') {
            if (data.type === 'apply')
                syncEmbeddableType(getEmbeddableType(data.options.embeddableType.id));
        ...
        // 修改
            ...
            if (data.type === 'apply')
                syncGroup(getGroup(data.options.group.id), data.revertOptions.group);
            else if (data.type === 'revert')
                syncGroup(getGroup(data.revertOptions.group.id), data.options.group);
        } else if (data.key === 'entity:change') {
            if (data.type === 'apply')
                syncEntity(getEntity(data.options.entity.id), data.revertOptions.entity);
            else if (data.type === 'revert')
                syncEntity(getEntity(data.revertOptions.entity.id), data.options.entity);
        } else if (data.key === 'mapped-super-class:change') {
            if (data.type === 'apply')
                syncMappedSuperClass(
                    getMappedSuperClass(data.options.mappedSuperClass.id),
                    data.revertOptions.mappedSuperClass,
                );
            else if (data.type === 'revert')
                syncMappedSuperClass(
                    getMappedSuperClass(data.revertOptions.mappedSuperClass.id),
                    data.options.mappedSuperClass,
                );
        } else if (data.key === 'embeddable-type:change') {
            if (data.type === 'apply')
                syncEmbeddableType(
                    getEmbeddableType(data.options.embeddableType.id),
                    data.revertOptions.embeddableType,
                );
            else if (data.type === 'revert')
                syncEmbeddableType(
                    getEmbeddableType(data.revertOptions.embeddableType.id),
                    data.options.embeddableType,
                );
        } else if (data.key === 'enumeration:change') {
            if (data.type === 'apply')
                syncEnumeration(
                    getEnumeration(data.options.enumeration.id),
                    data.revertOptions.enumeration,
                );
            else if (data.type === 'revert')
                syncEnumeration(
                    getEnumeration(data.revertOptions.enumeration.id),
                    data.options.enumeration,
                );
        } else if (data.key === 'association:change') {
            if (data.type === 'apply')
                syncAssociation(
                    getAssociation(data.options.association.id),
                    data.revertOptions.association,
                );
            else if (data.type === 'revert')
                syncAssociation(
                    getAssociation(data.revertOptions.association.id),
        ...
        // 导入
            ...
            if (data.type === 'apply') {
                ...
                for (const group of data.options.data.groups)
                    collectGroupNeedDiagnose(group, undefined, subIdSets);
                for (const {data: enumeration} of data.options.data.enumerations)
                    collectEnumerationNeedDiagnose(enumeration, undefined, subIdSets);
                for (const {data: embeddableType} of data.options.data.embeddableTypes)
                    collectEmbeddableTypeNeedDiagnose(embeddableType, undefined, subIdSets);
                for (const {data: mappedSuperClass} of data.options.data.mappedSuperClasses)
                    collectMappedSuperClassNeedDiagnose(mappedSuperClass, undefined, subIdSets);
                for (const {data: entity} of data.options.data.entities)
                    collectEntityNeedDiagnose(entity, undefined, subIdSets);
                for (const {data: association} of data.options.data.associations)
                    collectAssociationNeedDiagnose(association, undefined, subIdSets);
                syncDiagnoseByIdSets(subIdSets);
            } else {
    ...
    const diagnose = () => {
        ...
        for (const enumeration of contextData.enumerationMap.values())
            setEnumerationDiagnose(enumeration);
        for (const embeddableType of contextData.embeddableTypeMap.values())
