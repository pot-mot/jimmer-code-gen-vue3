...
export const oneToOneIdOnlyToAssociation = (
    ...
    const sourceProperty = sourceEntity.allProperties.find(
        (it) => it.id === idOnly.sourcePropertyId,
    );
    if (!sourceProperty) throw new Error(`[${idOnly.sourcePropertyId}] not found`);
    if (sourceProperty.category !== 'OneToOne_Source')
        throw new Error(`[${getPropertyView(sourceProperty)}] is not a OneToOne_Source`);
...
export const manyToOneIdOnlyToAssociation = (
    ...
    if (!sourceProperty) throw new Error(`[${idOnly.sourcePropertyId}] not found`);
    if (sourceProperty.category !== 'ManyToOne')
        throw new Error(`[${getPropertyView(sourceProperty)}] is not a ManyToOne`);
...
export const manyToManyIdOnlyToAssociation = (
    ...
    if (!sourceProperty) throw new Error(`[${idOnly.sourcePropertyId}] not found`);
    if (sourceProperty.category !== 'ManyToMany_Source')
        throw new Error(`[${getPropertyView(sourceProperty)}] is not a ManyToMany_Source`);
...
export const abstractOneToOneIdOnlyToAssociation = (
    ...
    const referencedEntity = entityMap.get(idOnly.referencedEntityId);
    if (!referencedEntity) throw new Error(`[${idOnly.referencedEntityId}] not found`);
    const sourceProperty = sourceAbstractEntity.allProperties.find(
        (it) => it.id === idOnly.sourcePropertyId,
    );
    if (!sourceProperty) throw new Error(`[${idOnly.sourcePropertyId}] not found`);
    if (sourceProperty.category !== 'OneToOne_Source')
