...
export const toOneToOneAbstract = async (association: DeepReadonly<AbstractAssociationIdOnly>) => {
    const {
        executeAsyncBatch,
        waitChangeSync,
        contextData,
    ...
    if (sourceProperty.category !== 'ManyToOne' && sourceProperty.category !== 'OneToOne_Source') {
