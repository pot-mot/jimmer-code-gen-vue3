...
export const sqlServerTableGenerator: TableGenerator = (
    ...
    for (const table of tables) {
        ...
        for (const foreignKey of table.foreignKeys) {
            ...
            template.append(
                ` (${foreignKey.columnRefs.map((it) => it.referencedColumnName).join(', ')})`,
            );
    ...
    for (const [
        tableName,
        {createTable, createIndexes, createForeignKeys, createChecks, addComments},
    ] of statementMap) {
