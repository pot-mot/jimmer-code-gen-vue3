...
export const javaMappedSuperClassGenerator: MappedSuperClassGenerator = (
    ...
    if (mappedSuperClass.directExtends.size > 1) {
        ...
        template.append(
            ` extends ${[...mappedSuperClass.directExtends].map((it) => it.name).join(', ')}`,
        );
    }
    ...
    result[`/java/${packageDirPath}${mappedSuperClass.name}.java`] = template.build({
        cleanEmptyLineIndent: true,
    });
