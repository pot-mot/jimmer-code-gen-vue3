...
export type ScriptsStore = {
    ...
    readonly getScriptInfos: (
        options?: ScriptFilterOptions,
    ) => DeepReadonly<{[key in ScriptTypeName]: ScriptInfo<key>[]}>;
    ...
    return Object.freeze({
        ...
        getScriptInfos: (options?: ScriptFilterOptions) => {
            ...
            for (const script of scriptInfoMap.values()) {
                ...
                if (
                    options?.databaseType !== undefined &&
                    script.databaseType !== 'ANY' &&
                    script.databaseType !== options.databaseType
                )
                    continue;
                if (
                    options?.jvmLanguage !== undefined &&
                    script.jvmLanguage !== 'ANY' &&
                    script.jvmLanguage !== options.jvmLanguage
                )
                    continue;
