...
export const useModelNameSets = (
    ...
    const syncGroup = (group: DeepReadonly<Group>) => {
        syncNameSetAndNameMap(group, [modelNameSets.groupNameSet], groupNameMap);
    };
    const removeGroup = (group: DeepReadonly<Group>) => {
        removeFromNameSetAndNameMap(group, [modelNameSets.groupNameSet], groupNameMap);
    };
    ...
    const syncEntity = (entity: DeepReadonly<EntityWithProperties>) => {
        syncNameSetAndNameMap(
            entity,
            [modelNameSets.groupItemNameSet, modelNameSets.entityNameSet],
            entityNameMap,
        );
    ...
    const syncMappedSuperClass = (
        ...
        syncNameSetAndNameMap(
            mappedSuperClass,
            [modelNameSets.groupItemNameSet, modelNameSets.mappedSuperClassNameSet],
    ...
    const syncEmbeddableType = (embeddableType: DeepReadonly<EmbeddableTypeWithProperties>) => {
        syncNameSetAndNameMap(
            embeddableType,
            [modelNameSets.groupItemNameSet, modelNameSets.embeddableTypeNameSet],
    ...
    const syncEnumeration = (enumeration: DeepReadonly<Enumeration>) => {
        syncNameSetAndNameMap(
            enumeration,
            [modelNameSets.groupItemNameSet, modelNameSets.enumerationNameSet],
            enumerationNameMap,
        );
    ...
    history.eventBus.on('change', (data) => {
        ...
        // 修改
            ...
            if (data.type === 'apply')
                ...
                syncMappedSuperClass(getMappedSuperClass(data.revertOptions.mappedSuperClass.id));
        } else if (data.key === 'embeddable-type:change') {
            if (data.type === 'apply')
                syncEmbeddableType(getEmbeddableType(data.options.embeddableType.id));
            else if (data.type === 'revert')
                syncEmbeddableType(getEmbeddableType(data.revertOptions.embeddableType.id));
        } else if (data.key === 'enumeration:change') {
            if (data.type === 'apply') syncEnumeration(getEnumeration(data.options.enumeration.id));
            else if (data.type === 'revert')
                syncEnumeration(getEnumeration(data.revertOptions.enumeration.id));
        } else if (data.key === 'association:change') {
            if (data.type === 'apply') syncAssociation(getAssociation(data.options.association.id));
            else if (data.type === 'revert')
                syncAssociation(getAssociation(data.revertOptions.association.id));
        }
