...
export const protectRepeatIds = (
    ...
    const {groups, entities, mappedSuperClasses, embeddableTypes, enumerations, associations} =
        graphData;
        ...
        if (contextData.entityMap.has(entity.id)) {
            ...
            for (const {
                ...
                for (const property of properties) {
                    if ('referencedEntityId' in property) {
                        if (property.referencedEntityId === entity.id)
                            property.referencedEntityId = newEntityId;
                    }
                }
            }
            for (const {
                data: {properties},
            } of mappedSuperClasses) {
                for (const property of properties) {
                    if ('referencedEntityId' in property) {
                        if (property.referencedEntityId === entity.id)
                            property.referencedEntityId = newEntityId;
            ...
            for (const {data: association} of associations) {
                if ('sourceEntityId' in association) {
                    if (association.sourceEntityId === entity.id)
                        association.sourceEntityId = newEntityId;
                }
                if (association.referencedEntityId === entity.id)
                    association.referencedEntityId = newEntityId;
            }
            entity.id = newEntityId;

            for (const property of entity.properties) {
                if (newPropertyIdMap.has(property.id))
                    throw new Error(`[${property.name}-${property.id}] is already existed`);
                newPropertyIdMap.set(property.id, {entityId: entity.id, property});
                const newPropertyId = createId('Property');
                for (const {data: association} of associations) {
                    if (association.sourcePropertyId === property.id)
                        association.sourcePropertyId = newPropertyId;
                }
    ...
    for (const {data: mappedSuperClass} of mappedSuperClasses) {
        if (contextData.mappedSuperClassMap.has(mappedSuperClass.id)) {
            ...
            for (const {
                data: {extendsIds, properties},
            } of entities) {
                for (let i = 0; i < extendsIds.length; i++) {
                    if (extendsIds[i] === mappedSuperClass.id)
                        extendsIds[i] = newMappedSuperClassId;
                }
                for (const property of properties) {
                    if ('referencedAbstractEntityId' in property) {
                        if (property.referencedAbstractEntityId === mappedSuperClass.id)
                            property.referencedAbstractEntityId = newMappedSuperClassId;
                    }
                }
            }
            for (const {
            ...
            for (const {data: association} of associations) {
                if ('sourceAbstractEntityId' in association) {
                    if (association.sourceAbstractEntityId === mappedSuperClass.id)
                        association.sourceAbstractEntityId = newMappedSuperClassId;
                }
            }
            ...
            for (const property of mappedSuperClass.properties) {
                if (newPropertyIdMap.has(property.id))
                    throw new Error(`[${property.name}-${property.id}] is already existed`);
                newPropertyIdMap.set(property.id, {
            ...
            for (const {
                ...
                for (const property of properties) {
                    if ('embeddableTypeId' in property) {
                        if (property.embeddableTypeId === embeddableType.id)
                            property.embeddableTypeId = newEmbeddableTypeId;
                    }
                }
            }
            for (const {
