import {
    parseEmbeddablePropertyPath,
    parsePropertyPath,
} from '@/modelEditor/utils/parsePropertyPath.ts';
        ...
        if (embeddableType === undefined) {
            ...
            if ('columnNameOverrides' in property) {
                for (const columnNameOverride of property.columnNameOverrides ?? []) {
                    const propertyPath = parseEmbeddablePropertyPath(
                        columnNameOverride.propertyPath.split('.'),
                        embeddableType,
                        context,
                    );
...
export const getPropertyDependencies = (
    ...
    const {existedDependencies, missingDependencies} = getEmbeddableTypePropertyDependencies(
        property,
        context,
    );
    ...
    if ('referencedEntityId' in property) {
        ...
        if (referencedEntity === undefined) {
            ...
            if ('mappedById' in property) {
                ...
                for (const {id} of referencedEntity.allProperties.filter(
                    (it) => it.category === 'OneToOne_Source',
                ))
                    sourcePropertyIds.push(id);
                for (const {id} of referencedEntity.allProperties.filter(
                    (it) => it.category === 'OneToOne_Mapped',
                ))
                    sourcePropertyIds.push(id);
                for (const {id} of referencedEntity.allProperties.filter(
                    (it) => it.category === 'ManyToOne',
                ))
                    sourcePropertyIds.push(id);
                ...
                if (!sourcePropertyIds.includes(property.mappedById)) {
                    missingDependencies.push({
                        path: ['mappedById'],
                        dependenceIds: {
                            propertyIds: [{entityId: referencedEntity.id, id: property.mappedById}],
                        },
                    });
                } else {
                    existedDependencies.push({
    ...
    if ('baseToManyPropertyId' in property) {
        const baseToManyProperty = entity.allProperties.find(
            (it) => it.id === property.baseToManyPropertyId,
        );
        if (!baseToManyProperty || !('referencedEntityId' in baseToManyProperty)) {
            missingDependencies.push({
                path: ['baseToManyPropertyId'],
                dependenceIds: {
                    propertyIds: [{entityId: entity.id, id: property.baseToManyPropertyId}],
                },
            });
        } else {
            existedDependencies.push({
            ...
            if (!context.entityMap.has(baseToManyProperty.referencedEntityId)) {
                ...
                const referencedEntity = context.entityMap.get(
                    baseToManyProperty.referencedEntityId,
                );
                if (!referencedEntity) {
                    missingDependencies.push({
                        path: ['entityId'],
                    ...
                    const deeperProperty =
                        ...
                        referencedEntity.manyToManySourcePropertyMap.get(
                            property.deeperPropertyId,
                        ) ??
...
export const getMappedSuperClassDependencies = (
    ...
    for (const property of mappedSuperClass.properties) {
        ...
        existedDependencies.push(
            ...propertyExistedDependencies.map((it) =>
                unshiftPath(['properties', property.id], it),
            ),
        );
        missingDependencies.push(
            ...propertyMissingDependencies.map((it) =>
                unshiftPath(['properties', property.id], it),
            ),
        );
    }
