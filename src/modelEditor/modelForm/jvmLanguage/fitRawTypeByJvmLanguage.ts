...
export const checkPropertyRawTypeIsFit = (
    ...
    if (currentLanguage === 'JAVA') {
        const mapping = javaKtTypeMapping.find((mapping) => {
            return mapping.ktType === property.rawType && property.nullable === mapping.nullable;
        });
        if (mapping !== undefined) return false;
    } else if (currentLanguage === 'KOTLIN') {
        const mapping = javaKtTypeMapping.find((mapping) => {
            return mapping.javaType === property.rawType && property.nullable === mapping.nullable;
        });
        if (mapping !== undefined) return false;
    }
...
const togglePropertyRawType = (property: Property, currentLanguage: JvmLanguage) => {
...
>;
export const stringifyUnfitRawType = (unfitRawType: UnfitRawTypeInfo): string => {
