...
const {contextData, copy, cut, remove} = useModelEditor();

const {close} = useModelContextMenu();

const {open: openGroupEditDialog} = useGroupEditDialog();
...
const {canPaste, handlePaste} = useModelEditorPaste(overrideGroupProducer(() => props.id));
