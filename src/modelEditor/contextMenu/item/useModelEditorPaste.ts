...
export const overrideGroupProducer = (getGroupId: () => string | undefined) => {
    return (data: Partial<ModelGraphSubData>) => {
        ...
        data.entities?.map((entity) => {
            entity.data.groupId = groupId;
        });
        data.mappedSuperClasses?.map((entity) => {
            entity.data.groupId = groupId;
        });
        data.embeddableTypes?.map((embeddableType) => {
            embeddableType.data.groupId = groupId;
        });
        data.enumerations?.map((enumeration) => {
            enumeration.data.groupId = groupId;
        });
    };
};

export const useModelEditorPaste = (producer?: (data: Partial<ModelGraphSubData>) => void) => {
