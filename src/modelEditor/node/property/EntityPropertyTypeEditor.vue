            ...
            if (!propertyIsId.value) {
                for (const entity of menuItem.entities.values()) {
                    if (
                        ...
                        entity.comment.toLowerCase().includes(keyword)
                    ) {
                        current.options.push({type: 'Entity', entity});
                    }
                }
...
const selectEntity = (entity: DeepReadonly<EntityWithProperties>) => {
    if (
        propertyIsId.value ||
        ('associationId' in property.value &&
            contextData.associationMap.has(property.value.associationId) &&
            'referencedEntityId' in property.value &&
            property.value.referencedEntityId === entity.id)
    )
        return;
    ...
    <Dropdown v-model="openState">
        <template #head>
            <div class="type-editor-header">
                ...
                <div
                    v-if="'referencedEntityId' in property"
                    class="type-editor-header-label"
                >
            ...
            >
                <span v-if="association.association.type === 'ManyToOne'">
                    <button @click.stop.prevent="toOneToOne(association.association)">Many</button>
                    To
                    <button @click.stop.prevent="toManyToMany(association.association)">One</button>
                </span>
                <span v-else-if="association.association.type === 'OneToOne'">
                    <button @click.stop.prevent="toManyToOne(association.association)">One</button>
                    To One
                </span>
                <span v-else-if="association.association.type === 'ManyToMany'">
                    Many To
                    <button @click.stop.prevent="toManyToOne(association.association)">Many</button>
                </span>
            </div>
        </template>
                        ...
                        <ul>
                            <template v-for="option in groupOptions.options">
                                ...
                                <li
                                    ...
                                    :class="{
                                        selected:
                                            'referencedEntityId' in property &&
                                            option.entity.id === property.referencedEntityId,
                                    }"
