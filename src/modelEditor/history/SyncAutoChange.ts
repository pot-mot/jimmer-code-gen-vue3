...
import {
    generateFkJoinInfo,
    generateMidTableJoinInfo,
} from '@/modelEditor/property/PropertyJoinInfoGenerate.ts';
import {
    tmpl_fkComment,
    tmpl_fkName,
    tmpl_idView,
        ...
        if ('joinInfo' in property && property.autoGenerateJoinInfo) {
            ...
            try {
                if (property.joinInfo.type !== 'MidTable') {
                    ...
                    property.joinInfo = generateMidTableJoinInfo(
                        property,
                        association,
                        contextData,
                    );
    ...
    if ('sourceEntityId' in association) {
        ...
        if (
            ...
            if ('useCommentTemplate' in association && association.useCommentTemplate) {
                association.comment = tmpl_fkComment(
            ...
            if ('useNameTemplate' in association && association.useNameTemplate) {
                association.name = tmpl_midTableName(
                    association.nameTemplate,
                    sourceEntity,
                    referencedEntity,
                );
            ...
            if ('useCommentTemplate' in association && association.useCommentTemplate) {
                association.comment = tmpl_midTableComment(
                    association.commentTemplate,
                    sourceEntity,
                    referencedEntity,
                );
        ...
        if ('useNameTemplate' in mappedProperty && mappedProperty.useNameTemplate) {
            mappedProperty.name = tmpl_mappedPropertyName(
        ...
        if ('useCommentTemplate' in mappedProperty && mappedProperty.useCommentTemplate) {
            mappedProperty.comment = tmpl_mappedPropertyComment(
        ...
        if ('useIdViewNameTemplate' in mappedProperty && mappedProperty.useIdViewNameTemplate) {
            mappedProperty.idViewName = tmpl_idView(
                mappedProperty.idViewNameTemplate,
                mappedProperty,
            );
        }
    }
