...
import {
    type MappedSuperClassNode,
    NodeType_MappedSuperClass,
} from '@/modelEditor/node/MappedSuperClassNode.ts';
import {
    type EmbeddableTypeNode,
    NodeType_EmbeddableType,
} from '@/modelEditor/node/EmbeddableTypeNode.ts';
...
import {
    ...
    fillModelSubIds,
    subIdsToSubIdSets,
} from '@/modelEditor/utils/ModelSubIds.ts';
...
import {getPropertiesAssociationChange} from '@/modelEditor/property/PropertyAssociationSync.ts';
...
export type ModelEditorHistoryCommands = {
    ...
    >;
    remove: CommandDefinition<ModelSubIds, ModelGraphSubData>;
};
...
export const useModelEditorHistory = (
    ...
    const inheritInfo = ref(
        defaultInheritInfo(getContextData().mappedSuperClassMap, getContextData().entityMap),
    );
    ...
    const addGroup = (options: {group: Group}) => {
        ...
        if (groupWatchStopMap.has(id)) throw new Error(`[${id}] is already watched`);
        if (menuMap.value.has(id))
            throw new Error(`Menu item [${id}] is already existed in menuMap`);
    ...
    const revertAddGroup = ({id}: DeepReadonly<{id: string}>) => {
        ...
        if (menuItem.mappedSuperClassMap.size > 0)
            throw new Error(`[${id}] has mapped super classes, can't remove`);
        if (menuItem.embeddableTypeMap.size > 0)
            throw new Error(`[${id}] has embeddable types, can't remove`);
        if (menuItem.enumerationMap.size > 0)
            throw new Error(`[${id}] has enumerations, can't remove`);
    ...
    const addEntity = (
        options: DeepReadonly<{entity: EntityWithProperties; position: XYPosition}>,
    ) => {
        ...
        if (menuItem.entityMap.has(id))
            throw new Error(`[${id}] is already existed in [${groupId}]`);
    ...
    const updateEntity = (options: DeepReadonly<{entity: EntityWithProperties}>) => {
        ...
        if (!contextData.groupMap.has(oldGroupId))
            throw new Error(`[${oldGroupId}] is not existed`);
        const menuItem = menuMap.value.get(oldGroupId);
        if (!menuItem) throw new Error(`[${oldGroupId}] is not existed in menuMap`);
        if (!menuItem.entityMap.has(id))
            throw new Error(`[${id}] is not existed in [${oldGroupId}]`);
        ...
        for (const edgedAssociation of contextData.associationMap.values()) {
            if (edgedAssociation.association.referencedEntityId === id) {
                ...
                'sourceEntityId' in edgedAssociation.association &&
                edgedAssociation.association.sourceEntityId === id
            ) {
                removeAssociationWatcher(edgedAssociation.association.id);
                const oldAssociation = cloneDeepReadonlyRaw<EdgedAssociation>(edgedAssociation);
                syncAssociationAutoChange(edgedAssociation.association, contextData);
        ...
        if (oldGroupId !== newGroupId) {
            ...
            if (newMenuItem.entityMap.has(id))
                throw new Error(`[${id}] is existed in [${newGroupId}]`);
            menuItem.entityMap.delete(id);
    ...
    const addMappedSuperClassWatcher = (id: string) => {
        ...
        mappedSuperClassOldMap.set(
            id,
            cloneDeepReadonlyRaw(contextData.mappedSuperClassMap.get(id)),
        );
        const debounceSyncMappedSuperClassUpdate = debounce(
            (newMappedSuperClass: MappedSuperClassWithProperties | undefined) => {
                const oldMappedSuperClass = mappedSuperClassOldMap.get(id);
                if (newMappedSuperClass && oldMappedSuperClass) {
                    history.executeBatch(
                        Symbol('mapped-super-class change with association sync'),
                        () => {
                            const {needRemoveIds} = getPropertiesAssociationChange(
        ...
        const stopWatch = watch(
            () => contextData.mappedSuperClassMap.get(id),
            (value) => {
                waitSyncIds.value.add(id);
    ...
    const addMappedSuperClass = (
        ...
        const menuItem = menuMap.value.get(groupId);
        if (!menuItem) throw new Error(`[${groupId}] is not existed in menuMap`);
        if (menuItem.mappedSuperClassMap.has(id))
            throw new Error(`[${id}] is already existed in [${groupId}]`);
    ...
    const revertAddMappedSuperClass = ({id}: DeepReadonly<{id: string}>) => {
        ...
        if (!menuItem) throw new Error(`[${groupId}] is not existed in menuMap`);
        if (!menuItem.mappedSuperClassMap.has(id))
            throw new Error(`[${id}] is not existed in [${groupId}]`);
    ...
    const updateMappedSuperClass = (
        options: DeepReadonly<{mappedSuperClass: MappedSuperClassWithProperties}>,
    ) => {
        const id = options.mappedSuperClass.id;
        const contextData = getContextData();
        const vueFlow = getVueFlow();
        ...
        for (const edgedAssociation of contextData.associationMap.values()) {
            if (
                'sourceAbstractEntityId' in edgedAssociation.association &&
                edgedAssociation.association.sourceAbstractEntityId === id
            ) {
                removeAssociationWatcher(edgedAssociation.association.id);
                const oldAssociation = cloneDeepReadonlyRaw<EdgedAssociation>(edgedAssociation);
                syncAssociationAutoChange(edgedAssociation.association, contextData);
        ...
        if (oldGroupId !== newGroupId) {
            const newMenuItem = menuMap.value.get(newGroupId);
            if (!newMenuItem) throw new Error(`[${newGroupId}] is not existed in menuMap`);
            if (newMenuItem.mappedSuperClassMap.has(id))
                throw new Error(`[${id}] is existed in [${newGroupId}]`);
            menuItem.mappedSuperClassMap.delete(id);
    ...
    const addEmbeddableTypeWatcher = (id: string) => {
        ...
        const debounceSyncEmbeddableTypeUpdate = debounce(
            (newEmbeddableType: EmbeddableTypeWithProperties | undefined) => {
    ...
    const addEmbeddableType = (
        ...
        const menuItem = menuMap.value.get(groupId);
        if (!menuItem) throw new Error(`[${groupId}] is not existed in menuMap`);
        if (menuItem.embeddableTypeMap.has(id))
            throw new Error(`[${id}] is already existed in [${groupId}]`);
    ...
    const updateEmbeddableType = (
        options: DeepReadonly<{embeddableType: EmbeddableTypeWithProperties}>,
    ) => {
        const id = options.embeddableType.id;
        const contextData = getContextData();
        const vueFlow = getVueFlow();
        ...
        const embeddableType = cloneDeepReadonlyRaw<EmbeddableTypeWithProperties>(
            options.embeddableType,
        );
        ...
        for (const property of embeddableType.properties) {
            if ('autoSyncColumnName' in property && property.autoSyncColumnName) {
                property.columnInfo.name = nameTool.convert(
                    property.name,
                    'UPPER_CAMEL',
        ...
        if (oldGroupId !== newGroupId) {
            const newMenuItem = menuMap.value.get(newGroupId);
            if (!newMenuItem) throw new Error(`[${newGroupId}] is not existed in menuMap`);
            if (newMenuItem.embeddableTypeMap.has(id))
                throw new Error(`[${id}] is existed in [${newGroupId}]`);
            menuItem.embeddableTypeMap.delete(id);
    ...
    const addEnumerationWatcher = (id: string) => {
        ...
        const debounceSyncEnumerationUpdate = debounce(
            (newEnumeration: Enumeration | undefined) => {
        ...
        const stopWatch = watch(
            () => contextData.enumerationMap.get(id),
            (value) => {
                waitSyncIds.value.add(id);
    ...
    const addEnumeration = (
        options: DeepReadonly<{enumeration: Enumeration; position: XYPosition}>,
    ) => {
        ...
        const menuItem = menuMap.value.get(groupId);
        if (!menuItem) throw new Error(`[${groupId}] is not existed in menuMap`);
        if (menuItem.enumerationMap.has(id))
            throw new Error(`[${id}] is already existed in [${groupId}]`);
    ...
    const updateEnumeration = (options: DeepReadonly<{enumeration: Enumeration}>) => {
        ...
        if (oldGroupId !== newGroupId) {
            const newMenuItem = menuMap.value.get(newGroupId);
            if (!newMenuItem) throw new Error(`[${newGroupId}] is not existed in menuMap`);
            if (newMenuItem.enumerationMap.has(id))
                throw new Error(`[${id}] is existed in [${newGroupId}]`);
            menuItem.enumerationMap.delete(id);
    ...
    const addAssociationWatcher = (id: string) => {
        ...
        if (associationWatchStopMap.has(id))
            throw new Error(`Association [${id}] is already watched`);

        associationOldMap.set(
            id,
            cloneDeepReadonlyRaw<EdgedAssociation | undefined>(contextData.associationMap.get(id)),
        );
        const debounceSyncAssociationUpdate = debounce(
            (newAssociation: EdgedAssociation | undefined) => {
                ...
                associationOldMap.set(
                    id,
                    cloneDeepReadonlyRaw<EdgedAssociation | undefined>(newAssociation),
                );
                waitSyncIds.value.delete(id);
            },
            SYNC_DEBOUNCE_TIMEOUT,
        ...
        const stopWatch = watch(
            () => contextData.associationMap.get(id),
            (value) => {
                waitSyncIds.value.add(id);
    ...
    const checkAssociationSourceUsed = (
        ...
        if ('sourceEntityId' in association) {
            for (const {association: existedAssociation} of contextData.associationMap.values()) {
                if (
                    association.id !== existedAssociation.id &&
                    association.sourcePropertyId === existedAssociation.sourcePropertyId
                ) {
                    return true;
                }
            }
    ...
    const getAssociationSource_CheckSourceUsed = (
        ...
        if ('sourceEntityId' in association) {
            ...
            for (const {association: existedAssociation} of contextData.associationMap.values()) {
                if (
                    existedAssociation.id !== association.id &&
                    existedAssociation.sourcePropertyId === association.sourcePropertyId
                ) {
                    const sourceProperty = sourceEntity.properties.find(
                        (property) => property.id === association.sourcePropertyId,
                    );
                    if (!sourceProperty)
                        throw new Error(`[${existedAssociation.sourcePropertyId}] is not existed`);
                    throw new Error(
                        `[${sourceProperty.name}] cannot used as source in two association`,
                    );
                }
            }
        } else {
            ...
            if (!sourceAbstractEntity)
                throw new Error(`[${association.sourceAbstractEntityId}] is not existed`);
            ...
            for (const {association: existedAssociation} of contextData.associationMap.values()) {
                if (
                    ...
                    const sourceProperty = sourceAbstractEntity.properties.find(
                        (property) => property.id === association.sourcePropertyId,
                    );
                    if (!sourceProperty)
                        throw new Error(`[${existedAssociation.sourcePropertyId}] is not existed`);
    ...
    const addAssociation = (options: DeepReadonly<EdgedAssociation>) => {
        ...
        if (contextData.associationMap.has(id))
            throw new Error(`Association [${id}] is already existed`);

        const {sourceNode, sourceHandleId} = getAssociationSource_CheckSourceUsed(
            options.association,
            contextData,
            vueFlow,
        );
    ...
    const revertAddAssociation = ({id}: DeepReadonly<{id: string}>) => {
        ...
        const association = cloneDeepReadonlyRaw<EdgedAssociation | undefined>(
            contextData.associationMap.get(id),
        );
    ...
    const importIntoContext = (
        ...
        try {
            ...
            for (const {data: mappedSuperClass, position} of graphData.mappedSuperClasses) {
                result.mappedSuperClassIds.push(
                    addMappedSuperClass({mappedSuperClass, position}).id,
                );
            }
    ...
    const removeFromContext = (modelSubIds: DeepReadonly<ModelSubIds>): ModelGraphSubData => {
        ...
        try {
            ...
            for (const entityId of idSets.entityIdSet) {
                for (const {association} of contextData.associationMap.values()) {
                    if (
                        'sourceEntityId' in association &&
                        association.sourceEntityId === entityId
                    ) {
            ...
            for (const mappedSuperClassId of idSets.mappedSuperClassIdSet) {
                for (const {association} of contextData.associationMap.values()) {
                    if (
                        'sourceAbstractEntityId' in association &&
                        association.sourceAbstractEntityId === mappedSuperClassId
                    ) {
                        idSets.associationIdSet.add(association.id);
                    }
                }
            ...
            for (const mappedSuperClassId of idSets.mappedSuperClassIdSet) {
                ...
                if (mappedSuperClass && node) {
                    result.mappedSuperClasses.push({
                        data: mappedSuperClass,
