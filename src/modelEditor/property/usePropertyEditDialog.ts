...
export const usePropertyEditDialog = createStore(() => {
    ...
    const {contextData, changeEntity, changeMappedSuperClass, changeEmbeddableType} =
        useModelEditor();
        ...
        submit(propertyWithSource: PropertyWithSource) {
            if (propertyWithSource) {
                if ('entityId' in propertyWithSource) {
                    ...
                    const property = propertyWithSource.property;
                    const mappedSuperClass = contextData.mappedSuperClassMap.get(
                        propertyWithSource.mappedSuperClassId,
                    );
                    if (mappedSuperClass) {
                        changeMappedSuperClass({
                            ...
                            properties: mappedSuperClass.properties.map((it) => {
                                if (it.id === property.id)
                                    return property as MappedSuperClassProperty;
                                return it;
                            }),
                        });
                    }
                    ...
                    const property = propertyWithSource.property;
                    const embeddableType = contextData.embeddableTypeMap.get(
                        propertyWithSource.embeddableTypeId,
                    );
                    if (embeddableType) {
                        changeEmbeddableType({
                            ...
                            properties: embeddableType.properties.map((it) => {
                                if (it.id === property.id)
                                    return property as EmbeddableTypeProperty;
                                return it;
                            }),
                        });
                    }
