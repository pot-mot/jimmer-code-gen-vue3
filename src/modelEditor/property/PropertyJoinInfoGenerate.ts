...
const _generateFkJoinInfo = (
    ...
    if (referencedIdProperties.length === 0)
        throw new Error(`[${referencedEntity.id}] has no id property`);
    if (referencedIdProperties.length > 1)
        throw new Error(`[${referencedEntity.id}] has more than one id properties`);
    const referencedIdProperty = referencedIdProperties[0];
    if (referencedIdProperty === undefined)
        throw new Error(`[${referencedEntity.id}] has no id property`);

    if (referencedIdProperty.category === 'ID_EMBEDDABLE') {
        const columnNames = flatEmbeddableTypeColumnNames(
            referencedIdProperty.embeddableTypeId,
            embeddableTypeMap,
            referencedIdProperty.columnNameOverrides,
        );
        return {
            type: 'MultiColumn',
            embeddableTypeId: referencedIdProperty.embeddableTypeId,
            columnRefs: columnNames.map((columnName) => ({
                columnName: nameTool.convert(
                    referencedEntity.name +
                        nameTool.convert(columnName, databaseNameStrategy, 'UPPER_CAMEL'),
                    'UPPER_CAMEL',
...
const _generateMidTableJoinInfo = (
    ...
    if (
        ...
        if (sourceIdProperties.length === 0)
            throw new Error(`[${sourceEntity.id}] has no id property`);
        if (sourceIdProperties.length > 1)
            throw new Error(`[${sourceEntity.id}] has more than one id properties`);
        const sourceIdProperty = sourceIdProperties[0];
        if (sourceIdProperty === undefined)
            throw new Error(`[${sourceEntity.id}] has no id property`);
        ...
        if ('embeddableTypeId' in sourceIdProperty) {
            const sourceColumnNames = flatEmbeddableTypeColumnNames(
                sourceIdProperty.embeddableTypeId,
                embeddableTypeMap,
                sourceIdProperty.columnNameOverrides,
            );
            sourceJoinInfo = {
                type: 'MultiColumn',
        ...
        if ('embeddableTypeId' in referencedIdProperty) {
            const targetColumnNames = flatEmbeddableTypeColumnNames(
                referencedIdProperty.embeddableTypeId,
                embeddableTypeMap,
                referencedIdProperty.columnNameOverrides,
            );
