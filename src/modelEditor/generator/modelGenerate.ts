...
export const modelGenerate = (
    ...
    if (selectedIds === undefined) {
        ...
        if (selectedIds.entityIds)
            entities = getArrayFromMap(context.entityMap, selectedIds.entityIds);
        if (selectedIds.mappedSuperClassIds)
            mappedSuperClasses = getArrayFromMap(
                context.mappedSuperClassMap,
                selectedIds.mappedSuperClassIds,
            );
        if (selectedIds.embeddableTypeIds)
            embeddableTypes = getArrayFromMap(
                context.embeddableTypeMap,
                selectedIds.embeddableTypeIds,
            );
        if (selectedIds.enumerationIds)
            enumerations = getArrayFromMap(context.enumerationMap, selectedIds.enumerationIds);
        if (selectedIds.associationIds)
            associations = getArrayFromMap(context.associationMap, selectedIds.associationIds);
        if (selectedIds.groupIds) groups = getArrayFromMap(context.groupMap, selectedIds.groupIds);
    }

    if (entities) {
        const {tables, midTables} = entityToTable(
            entities,
            Array.from(context.entityMap.values()),
            context,
        );
