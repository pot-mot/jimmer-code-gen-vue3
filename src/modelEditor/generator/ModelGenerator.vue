...
const {getContext, selectedIdSets} = useModelEditor();

const {openState, generate} = useModelGenerator();

const {open: openGenerateScriptEditor} = useScriptDialog();
...
const handleGenerate = async () => {
    ...
    await withLoading('Generating...', async () => {
        try {
            const generateResult = generate(getContext(), subIdSetToSubIds(selectedIdSets.value));
...
const handleDownloadFiles = (selectedPathSet?: Set<string>) => {
    try {
        withLoading(translate('download'), async () => {
            if (
                generateFiles.value === undefined ||
                Object.keys(generateFiles.value).length === 0
            ) {
                return;
            }
        ...
        >
            ...
            >
                <div class="error-script">
                    {{ translate({key: 'script_execute_error', args: [error.scriptName]}) }}
                </div>
                <div
                    class="error-type"
                    v-if="error.target.type === 'Entity'"
                >
                    <EntityViewer :entity="error.target.entity" />
                </div>
                <div
                    class="error-type"
                    v-else-if="error.target.type === 'MappedSuperClass'"
                >
                    <MappedSuperClassViewer :mapped-super-class="error.target.mappedSuperClass" />
                </div>
                <div
                    class="error-type"
                    v-else-if="error.target.type === 'EmbeddableType'"
                >
                    <EmbeddableTypeViewer :embeddable-type="error.target.embeddableType" />
                </div>
                <div
                    class="error-type"
                    v-else-if="error.target.type === 'Enumeration'"
                >
                    <EnumerationViewer :enumeration="error.target.enumeration" />
                </div>
                <div
                    class="error-type"
                    v-else-if="error.target.type === 'Association'"
                >
            ...
            <template #left-top="{selectedPathSet}">
                <div class="file-toolbar">
                    ...
                    <button
                        @click="
                            handleDownloadFiles(
                                downloadScope === 'ALL' ? undefined : selectedPathSet,
                            )
                        "
                    >
                        <IconDownload />
