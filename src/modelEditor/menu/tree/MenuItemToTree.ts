...
const mappedSuperClassToTreeNode = (
    mappedSuperClass: MappedSuperClassWithProperties,
): MenuItemTreeNode => {
    return {
...
const getDataName = (item: MenuItemTreeNode): string => {
    switch (item.data.type) {
        ...
        case 'Association':
            return 'name' in item.data.association
                ? item.data.association.name
                : item.data.association.nameTemplate;
    }
};

export const menuItemToTree = (
    menuItem: MenuItem,
    filter: (item: {name: string; comment: string}) => boolean = () => true,
): MenuItemTreeNode => {
    return {
