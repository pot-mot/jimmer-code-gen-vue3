...
type CommonPropertyItem = {
    ...
    property: DeepReadonly<
        IdCommonProperty | ScalarCommonProperty | ScalarEnumProperty | VersionProperty
    >;
};
...
export const parseEmbeddablePropertyPath = (
    ...
    if (!currentProperty) {
        throw new Error(
            `PropertyPath [${propertyPathNames}] is not a valid association path because [${propertyPathNames[0]}] not existed`,
        );
    }
    ...
    for (let i = 0; i < propertyPathNames.length - 1; i++) {
        ...
        if ('embeddableTypeId' in currentProperty) {
            ...
            const nextProperty = embeddableType.properties.find(
                (it) => it.name === nextPropertyName,
            );
            if (!nextProperty) {
                throw new Error(
                    `[${nextPropertyName}] is not found in [${currentProperty.embeddableTypeId}]`,
                );
            }
            path.push({
                type: 'EmbeddableType',
            ...
            throw new Error(
                `PropertyPath [${propertyPathNames}] is not a valid association path because [${propertyPathNames[i]}] is not a EmbeddableProperty`,
            );
        }
    }
    ...
    if (
        ...
        throw new Error(
            `PropertyPath [${propertyPathNames}] is not a valid association path because [${propertyPathNames[propertyPathNames.length - 1]}] is not a Scalar`,
        );
...
export const parsePropertyPath = (
    ...
    let currentProperty = entity.allProperties.find(
        (it) => 'name' in it && it.name === propertyPathNames[0],
    );
    if (!currentProperty) {
        throw new Error(
            `PropertyPath [${propertyPathNames}] is not a valid association path because [${propertyPathNames[0]}] not existed`,
    ...
    for (let i = 0; i < propertyPathNames.length - 1; i++) {
        ...
        if (
            ...
            const nextProperty = nextEntity.allProperties.find(
                (it) => 'name' in it && it.name === nextPropertyName,
            );
            if (!nextProperty) {
                throw new Error(
                    `[${nextPropertyName}] is not found in [${currentProperty.referencedEntityId}]`,
                );
            }
            path.push({
                type: 'Entity',
        ...
        // 检查是否是 Embeddable 内嵌属性
            ...
            throw new Error(
                `PropertyPath [${propertyPathNames}] is not a valid association path because [${propertyPathNames[i]}] is not a ToOne ReferenceProperty or EmbeddableProperty`,
            );
        }
    }
    ...
    if (
        ...
        throw new Error(
            `PropertyPath [${propertyPathNames}] is not a valid association path because [${propertyPathNames[propertyPathNames.length - 1]}] is not a Scalar or IdProperty`,
        );
    } else if (path.length === 0) {
        path.push({
            type: 'COMMON',
