export const createJvmFileBuilder = (
    ...
    const {groupId, subPackagePath} = options;
    ...
    const requireEnumeration = (id: string) => {
        ...
        if (enumeration.groupId !== groupId) {
            ...
            if (enumerationGroup === undefined)
                throw new Error(`[${enumeration.groupId}] not found`);
            enumerationPath = buildPointPath([
                enumerationGroup.basePackagePath,
                enumeration.subPackagePath,
            ]);
        } else {
    ...
    const requireEmbeddableType = (id: string) => {
        ...
        if (embeddableType.groupId !== groupId) {
            ...
            if (embeddableTypeGroup === undefined)
                throw new Error(`[${embeddableType.groupId}] not found`);
            embeddableTypePath = buildPointPath([
                embeddableTypeGroup.basePackagePath,
                embeddableType.subPackagePath,
            ]);
        } else {
            embeddableTypePath = buildPointPath([
    ...
    const requireEntity = (id: string) => {
        ...
        if (entityPath !== packagePath) importSet.add(buildPointPath([entityPath, entity.name]));
    ...
    const useMappedSuperClassIdMap = new Map<
        string,
        DeepReadonly<MappedSuperClassWithInheritInfo>
    >();
    const requireMappedSuperClass = (id: string) => {
        ...
        if (mappedSuperClass.groupId !== groupId) {
            ...
            if (mappedSuperClassGroup === undefined)
                throw new Error(`[${mappedSuperClass.groupId}] not found`);
            mappedSuperClassPath = buildPointPath([
                mappedSuperClassGroup.basePackagePath,
                mappedSuperClass.subPackagePath,
            ]);
        } else {
            mappedSuperClassPath = buildPointPath([
                group.basePackagePath,
    ...
    const pushIdViewProperty = (
        ...
        if (referencedId.category === 'ID_COMMON') {
            propertyInfos.push({
                ...
                type: property.typeIsList
                    ? 'List<' + referencedId.rawType + '>'
                    : referencedId.rawType,
                annotations,
            ...
            propertyInfos.push({
                ...
                comment: property.comment,
                type: property.typeIsList
                    ? 'List<' + referencedEmbeddableType.name + '>'
                    : referencedEmbeddableType.name,
                annotations,
                nullable: property.nullable,
            });
        }
    ...
    const getAssociation = (associationId: string): DeepReadonly<Association> => {
            ...
            if (joinInfo.type === 'SingleColumn') {
                ...
                if (joinInfo.columnName && joinInfo.foreignKeyType !== defaultForeignKeyType) {
                    ...
                    annotationBuilder.appendLine(
                        `type = ForeignKeyType.${joinInfo.foreignKeyType}`,
                    );
    ...
    const generateKeyAnnotation = (keyGroups: ReadonlyArray<string>): string[] => {
        const annotations = [];
    ...
    const pushProperty = (
        ...
        if (property.category === 'ID_COMMON') {
            ...
            if (property.typeIsArray) {
                annotations.push(
                    `@Column(name = "${property.columnInfo.name}", sqlElementType = "${property.columnInfo.type}")`,
                );
            } else {
                annotations.push(`@Column(name = "${property.columnInfo.name}")`);
            }
            ...
            const annotations = ['@OneToOne'];
            if ('key' in property) {
                importSet.add('org.babyfish.jimmer.sql.Key');
                annotations.push(...generateKeyAnnotation(property.keyGroups));
            }
            const joinAnnotation = buildJoinAnnotation(
                property.joinInfo,
                getAssociation(property.associationId),
            );
            ...
            importSet.add('org.babyfish.jimmer.sql.OneToOne');
            const referencedEntity = requireEntity(property.referencedEntityId);
            const sourceProperty = referencedEntity.oneToOneSourcePropertyMap.get(
                property.mappedById,
            );
            ...
            importSet.add('org.babyfish.jimmer.sql.ManyToMany');
            const referencedEntity = requireEntity(property.referencedEntityId);
            const sourceProperty = referencedEntity.manyToManySourcePropertyMap.get(
                property.mappedById,
            );
            if (sourceProperty === undefined) throw new Error(`[${property.mappedById}] not found`);
            ...
            if (!('entity' in owner) && !('mappedSuperClass' in owner))
                throw new Error(`[${property.name}] owner is not an entity`);
            ...
            const dependencyString = property.dependencies
                .map((dependency) => {
                    const dependencyProperty = ownerProperties.find((p) => p.id === dependency);
                    if (dependencyProperty === undefined)
                        throw new Error(`[${dependency}] not found`);
                    if (!('name' in dependencyProperty))
                        throw new Error(`[${dependency}] not found`);
            ...
            if ('referencedEntityId' in property) {
                const referencedEntity = requireEntity(property.referencedEntityId);
                typeName = property.typeIsList
                    ? `List<${referencedEntity.name}>`
            ...
            if (baseToManyProperty === undefined)
                throw new Error(`[${property.baseToManyPropertyId}] not found`);
            const referencedEntity = requireEntity(baseToManyProperty.referencedEntityId);
            const deeperProperty = referencedEntity.allProperties.find(
                (prop) => prop.id === property.deeperPropertyId,
            );
            if (deeperProperty === undefined)
                throw new Error(
                    `[${referencedEntity.name}.${property.deeperPropertyId}] not found`,
                );
            if (!('name' in deeperProperty))
                throw new Error(
                    `[${referencedEntity.name}.${deeperProperty.nameTemplate}] not a concrete property`,
                );

            importSet.add('org.babyfish.jimmer.sql.ManyToManyView');
            annotations.push(
                `@ManyToManyView(prop = "${baseToManyProperty.name}", deeperProp = "${deeperProperty.name}")`,
            );
            annotations.push(...property.extraAnnotations);
    ...
    return {
        ...
        addImports: (imports: string | Iterable<string>) => {
            if (typeof imports === 'string') {
                ...
                for (const importItem of imports) importSet.add(importItem);
            }
        },
        requireEnumeration,
