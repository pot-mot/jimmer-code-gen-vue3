...
export const graphDataToModelData = (
    ...
    for (const {data: mappedSuperClass} of graphData.mappedSuperClasses) {
        if (entityMap.has(mappedSuperClass.id))
            throw new Error(`MappedSuperClass ${mappedSuperClass.id} already exists`);
    ...
    return {
        ...
        associations: graphData.associations.map((it) => ({
            association: it.data,
            labelPosition: it.labelPosition,
        })),
    };
};
