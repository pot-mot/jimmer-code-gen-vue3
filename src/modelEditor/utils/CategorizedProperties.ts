...
const defaultEntityCategorizedProperties = (): Omit<EntityCategorizedProperties, 'idProperty'> => {
    return {
        ...
        defaultOrderPropertyMap: new Map<
            string,
            Property & {defaultOrderDirection?: OrderDirection}
        >(),
...
const defaultAbstractCategorizedProperties = (): AbstractCategorizedProperties => {
    return {
        ...
        keyPropertyMap: new Map<
            string,
            (ScalarCommonProperty | ScalarEnumProperty) & KeyProperty
        >(),
        logicalDeleteProperty: undefined,
        versionProperty: undefined,
        defaultOrderPropertyMap: new Map<
    ...
    for (const property of properties) {
        if (property.category === 'ID_COMMON' || property.category === 'ID_EMBEDDABLE') {
            if (idProperty !== undefined) {
                throw new Error(
                    `Multiple idProperties: ${property.name}(${property.id}), ${idProperty.name}(${idProperty.id})`,
                );
            }
            idProperty = property;
            ...
            if (result.versionProperty !== undefined) {
                throw new Error(
                    `Multiple versionProperty: ${property.name}(${property.id}), ${result.versionProperty.name}(${result.versionProperty.id})`,
                );
            }
            ...
            if (result.scalarCommonPropertyMap.has(property.id)) {
                throw new Error(
                    `Duplicate scalarCommonProperty id (${property.id}): ${property.name}, ${result.scalarCommonPropertyMap.get(property.id)?.name}`,
                );
            }
            ...
            if (result.scalarEnumPropertyMap.has(property.id)) {
                throw new Error(
                    `Duplicate scalarEnumProperty id (${property.id}): ${property.name}, ${result.scalarEnumPropertyMap.get(property.id)?.name}`,
                );
            }
            ...
            if (result.scalarEmbeddablePropertyMap.has(property.id)) {
                throw new Error(
                    `Duplicate scalarEmbeddableProperty id (${property.id}): ${property.name}, ${result.scalarEmbeddablePropertyMap.get(property.id)?.name}`,
                );
            }
            ...
            if (result.manyToManyViewPropertyMap.has(property.id)) {
                throw new Error(
                    `Duplicate manyToManyViewProperty id (${property.id}): ${property.name}, ${result.manyToManyViewPropertyMap.get(property.id)?.name}`,
                );
            }
            ...
            if (result.getterFormulaPropertyMap.has(property.id)) {
                throw new Error(
                    `Duplicate getterFormulaProperty id (${property.id}): ${property.name}, ${result.getterFormulaPropertyMap.get(property.id)?.name}`,
                );
            }
            ...
            if (result.sqlFormulaPropertyMap.has(property.id)) {
                throw new Error(
                    `Duplicate sqlFormulaProperty id (${property.id}): ${property.name}, ${result.sqlFormulaPropertyMap.get(property.id)?.name}`,
                );
            }
            ...
            if (result.transientPropertyMap.has(property.id)) {
                throw new Error(
                    `Duplicate transientProperty id (${property.id}): ${property.name}, ${result.transientPropertyMap.get(property.id)?.name}`,
                );
            }
            ...
            if (result.oneToOneSourcePropertyMap.has(property.id)) {
                throw new Error(
                    `Duplicate oneToOneSourceProperty id (${property.id}): ${property.name}, ${result.oneToOneSourcePropertyMap.get(property.id)?.name}`,
                );
            }
            ...
            if (association.type !== 'OneToOne')
                throw new Error(`[${getAssociationView(association)}] is not OneToOne`);
            ...
            if (result.oneToOneMappedPropertyMap.has(property.id)) {
                throw new Error(
                    `Duplicate oneToOneTargetProperty id (${property.id}): ${property.name}, ${result.oneToOneMappedPropertyMap.get(property.id)?.name}`,
                );
            }
            const association = associationMap.get(property.associationId);
            if (association === undefined) throw new Error(`[${property.associationId}] not found`);
            ...
            if (result.manyToOnePropertyMap.has(property.id)) {
                throw new Error(
                    `Duplicate manyToOneProperty id (${property.id}): ${property.name}, ${result.manyToOnePropertyMap.get(property.id)?.name}`,
                );
            }
            const association = associationMap.get(property.associationId);
            if (association === undefined) throw new Error(`[${property.associationId}] not found`);
            if (association.type !== 'ManyToOne')
                throw new Error(`[${getAssociationView(association)}] is not ManyToOne`);
            ...
            if (result.oneToManyPropertyMap.has(property.id)) {
                throw new Error(
                    `Duplicate oneToManyProperty id (${property.id}): ${property.name}, ${result.oneToManyPropertyMap.get(property.id)?.name}`,
                );
            }
            const association = associationMap.get(property.associationId);
            if (association === undefined) throw new Error(`[${property.associationId}] not found`);
            ...
            if (result.manyToManySourcePropertyMap.has(property.id)) {
                throw new Error(
                    `Duplicate manyToManySourceProperty id (${property.id}): ${property.name}, ${result.manyToManySourcePropertyMap.get(property.id)?.name}`,
                );
            }
            const association = associationMap.get(property.associationId);
            if (association === undefined) throw new Error(`[${property.associationId}] not found`);
            if (association.type !== 'ManyToMany')
                throw new Error(`[${getAssociationView(association)}] is not ManyToMany`);
            ...
            if (result.manyToManyMappedPropertyMap.has(property.id)) {
                throw new Error(
                    `Duplicate manyToManyTargetProperty id (${property.id}): ${property.name}, ${result.manyToManyMappedPropertyMap.get(property.id)?.name}`,
                );
            }
            const association = associationMap.get(property.associationId);
            if (association === undefined) throw new Error(`[${property.associationId}] not found`);
        ...
        if ('key' in property && property.key) {
            if (result.keyPropertyMap.has(property.id)) {
                throw new Error(
                    `Duplicate keyProperty id (${property.id}): ${property.name}, ${getPropertyView(result.keyPropertyMap.get(property.id))}`,
                );
            }
            ...
            if (result.logicalDeleteProperty !== undefined) {
                throw new Error(
                    `Multiple logicalDeleteProperty: ${property.name}(${property.id}), ${getPropertyView(result.logicalDeleteProperty)}(${result.logicalDeleteProperty.id})`,
                );
            }
        ...
        if ('defaultOrderDirection' in property) {
            if (result.defaultOrderPropertyMap.has(property.id)) {
                throw new Error(
                    `Duplicate defaultOrderProperty id (${property.id}): ${property.name}, ${getPropertyView(result.defaultOrderPropertyMap.get(property.id))}`,
                );
            }
    ...
    for (const property of properties) {
        if (property.category === 'ID_COMMON' || property.category === 'ID_EMBEDDABLE') {
            if (result.idProperty !== undefined) {
                throw new Error(
                    `Multiple idProperties: ${property.name}(${property.id}), ${result.idProperty.name}(${result.idProperty.id})`,
                );
            }
            ...
            const association = associationMap.get(property.associationId);
            if (association === undefined) throw new Error(`[${property.associationId}] not found`);
            if (association.type !== 'OneToOne_Abstract')
                throw new Error(`[${getAssociationView(association)}] is not OneToOne`);
            result.oneToOneSourcePropertyMap.set(property.id, {...property, association});
        } else if (property.category === 'ManyToOne') {
            if (result.manyToOnePropertyMap.has(property.id)) {
            ...
            const association = associationMap.get(property.associationId);
            if (association === undefined) throw new Error(`[${property.associationId}] not found`);
            if (association.type !== 'ManyToOne_Abstract')
                throw new Error(`[${getAssociationView(association)}] is not ManyToOne`);
            result.manyToOnePropertyMap.set(property.id, {...property, association});
        }
