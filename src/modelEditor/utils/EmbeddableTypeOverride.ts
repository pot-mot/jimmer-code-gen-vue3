    ...
    const processEmbeddableProperties = (
        ...
        for (const property of properties) {
            ...
            if ('embeddableTypeId' in property) {
                ...
                const matchedOverride = columnNameOverrides.find(
                    (override) => override.propertyPath === propertyPath, // FIXME 内嵌属性的列名覆盖应该如何与 propertyPath 匹配
                );
                result.push({
                    propertyPath,
                    overrideColumnName: matchedOverride
                        ? matchedOverride.overrideColumnName
    ...
    const processEmbeddableProperties = (
        ...
        for (const property of properties) {
            ...
            if ('embeddableTypeId' in property) {
                ...
                const matchedOverride = columnNameOverrides.find(
                    (override) => propertyPath.endsWith(override.propertyPath), // FIXME 内嵌属性的列名覆盖应该如何与 propertyPath 匹配
                );
