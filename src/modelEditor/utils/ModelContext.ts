...
export const contextDataToContext = (
    ...
    for (const entity of tempEntityMap.values()) {
        ...
        for (const {association, mappedProperty} of mappedInfos) {
            ...
            if (!referencedEntity)
                throw new Error(`[${mappedProperty.referencedEntityId}] not existed`);
    ...
    for (const mappedSuperClass of tempMappedSuperClassMap.values()) {
        ...
        const mappedProperties = getAbstractMappedPropertyInfo(
            mappedSuperClass.properties,
            associationIdOnlyMap,
        );
        for (const {
            ...
            if (!referencedEntity)
                throw new Error(`[${abstractAssociation.referencedEntityId}] not existed`);
    ...
    for (const [id, mappedSuperClass] of tempMappedSuperClassMap) {
        ...
        for (const ancestorId of inheritItem.ancestorIdSet) {
            ...
            if (ancestor) {
                allProperties.push(
                    ...ancestor.properties.map((property) => ({
                        ...property,
    ...
    for (const [id, embeddableType] of embeddableTypeBaseInfoMap) {
        const overrideColumnNameProperties = overrideEmbeddableTypePropertiesColumnNames(
            embeddableType,
            embeddableTypeBaseInfoMap,
        );
        embeddableTypeMap.set(id, {
            ...embeddableType,
            overrideColumnNameProperties,
            categorizedOverrideColumnNameProperties: categorizeEmbeddableTypeProperties(
                overrideColumnNameProperties,
            ),
        });
    }
    ...
    for (const [id, idOnly] of associationIdOnlyMap) {
        const association = idOnlyToAssociation(
            idOnly,
    ...
    const context: ModelContext = {
        ...
        createJvmFileBuilder: (options: {groupId: string; subPackagePath: string}) => {
