...
export const tmpl_fkName = (
    ...
    return template
        .replace(
            SOURCE_ENTITY,
            nameTool.convert(sourceEntity.name, 'UPPER_CAMEL', getDatabaseNameStrategy()),
        )
        .replace(
            SOURCE_PROPERTY,
            nameTool.convert(sourceProperty.name, 'LOWER_CAMEL', getDatabaseNameStrategy()),
        );
};
...
export const tmpl_midTableName = (
    ...
    return template
        ...
        .replace(
            REFERENCED_ENTITY,
            nameTool.convert(referencedEntity.name, 'UPPER_CAMEL', getDatabaseNameStrategy()),
        );
};
...
export const tmpl_idView = (template: string, property: {name: string}) => {
    let propertyName = property.name;
    if (property.name.endsWith('List'))
        propertyName = propertyName.substring(0, propertyName.length - 4);

    return nameTool.convert(template.replace(PROPERTY, propertyName), 'LOWER_CAMEL', 'LOWER_CAMEL');
};
