...
import {
    fillModelGraphSubData,
    modelSubDataToGraphSubData,
} from '@/modelEditor/utils/ModelGraphSubData.ts';
import {
    contextDataGetSelectSubData,
    contextDataToSubData,
} from '@/modelEditor/utils/ModelSubData.ts';
...
export const CreateType_CONSTANTS = [
    'Entity',
    'MappedSuperClass',
    'EmbeddableType',
    'Enumeration',
...
export const useModelEditor = createStore(() => {
    ...
    const {history, canUndo, canRedo, menuMap, inheritInfo, waitChangeSync} = useModelEditorHistory(
        getContextData,
        getVueFlow,
    );
    ...
    const {modelDiagnoseInfo, diagnose} = useModelDiagnoseInfo(
        getContextData(),
        inheritInfo.value,
        modelNameSets,
        history,
    ...
    const getModelGraphSubData = (options?: {selected?: boolean}): ModelGraphSubData => {
        ...
        if (options?.selected) {
            return modelSubDataToGraphSubData(
                contextDataGetSelectSubData(contextData, modelSelection.selectedIdSets.value),
                vueFlow,
            );
        } else {
    ...
    const importModelGraphData = async (
        ...
        const mappedSuperClassIdSet = new Set<string>(
            fullData.mappedSuperClasses.map((it) => it.data.id),
        );
        for (const mappedSuperClassId of contextData.mappedSuperClassMap.keys())
            mappedSuperClassIdSet.add(mappedSuperClassId);
        fullData.associations = fullData.associations.filter(({data}) => {
            if ('sourceEntityId' in data) {
                return (
                    entityIdSet.has(data.sourceEntityId) && entityIdSet.has(data.referencedEntityId)
                );
            } else {
                return (
                    mappedSuperClassIdSet.has(data.sourceAbstractEntityId) &&
                    entityIdSet.has(data.referencedEntityId)
                );
            }
        });
        ...
        const startPosition = options?.fitCurrentScreenPosition
    ...
    const loadTables = async (tables: DeepReadonly<Table[]>) => {
        await withLoading('Table Loading...', async () => {
            ...
            const contextData = getContextData();
            const vueFlow = getVueFlow();
            const {entities, embeddableTypes, associations} = tableToEntity(
                tables,
                contextData.model.databaseNameStrategy,
                getCurrentGroupIdOrCreate(),
                createSqlToJvm(
                    getSqlToJvmMappingRules(),
                    contextData.model.jvmLanguage,
                    contextData.model.databaseType,
                ),
            );
            ...
            history.executeBatch(Symbol('load tables'), async () => {
                ...
                for (const embeddableType of embeddableTypes) {
                    embeddableTypeIds.push(
                        addEmbeddableType(embeddableType.groupId, embeddableType, {x: 0, y: 0}),
                    );
                }
                ...
                const {x: startX, y: startY} = vueFlow.screenToFlowCoordinate({
                    x:
                        vueFlow.dimensions.value.width * 0.15 +
                        (vueFlow.vueFlowRef.value?.getBoundingClientRect().left ?? 0),
    ...
    const addEnumeration = (
        groupId: string = getCurrentGroupIdOrCreate(),
        enumeration: Enumeration = defaultEnumeration(
            groupId,
            contextData.model.defaultEnumerationStrategy,
        ),
        position: XYPosition,
    ) => {
    ...
    const changeAssociation = (association: DeepReadonly<AssociationIdOnly>) => {
        ...
        history.executeCommand('association:change', {
            association,
            labelPosition: edgedAssociation.labelPosition,
        });
    };

    const remove = (ids: Partial<ModelSubIds>) => {
    ...
    // Selection 选中部分的图数据
    const modelSelection = useModelEditorSelectIds({
        getContextData,
        getVueFlow,
        getNextZIndex,
        history,
    ...
    // 默认操作为拖拽
    const setDefaultPanDrag = (vueFlow: VueFlowStore = getVueFlow()) => {
    ...
    // 默认操作为框选，通过鼠标右键拖拽
    const setDefaultSelectionRect = (vueFlow: VueFlowStore = getVueFlow()) => {
        defaultMouseAction.value = 'selectionRect';
        vueFlow.panOnDrag.value = [2];
        selectionRectMouseButton = 0;
        selectionRectEnable = true;
    };
    const toggleDefaultMouseAction = (vueFlow: VueFlowStore = getVueFlow()) => {
    ...
    const disableDrag = (vueFlow: VueFlowStore = getVueFlow()) => {
        vueFlow.nodesDraggable.value = false;
    };
    const enableDrag = (vueFlow: VueFlowStore = getVueFlow()) => {
        vueFlow.nodesDraggable.value = true;
    };

    const setLayerConfigDefault = (vueFlow: VueFlowStore = getVueFlow()) => {
        disableMultiSelect(vueFlow);
        setDefaultPanDrag(vueFlow);
        enableDrag(vueFlow);
    };
    ...
    const selectEdge = (edge: GraphEdge | string) => {
        ...
        if (
            _edge.type === EdgeType_ConcreteAssociation ||
            _edge.type === EdgeType_AbstractAssociation
        ) {
    ...
    const focusEdgeAndFitSub = async (edge: GraphEdge | string, subId: string) => {
        ...
        if (element !== null) {
            ...
            if (associationElement !== null) {
                await vueFlow.fitBounds(
                    getInnerElementRect(associationElement),
                    FIT_BOUND_DEFAULT_OPTION,
                );
            }
        }
    ...
    const focusEntityProperty = async (source: {entityId: string; propertyId: string}) => {
        ...
        await focusNodeAndFitSub(
            source.entityId,
            nodeSubElementId(source.entityId, source.propertyId),
        );
    };
    const focusMappedSuperClassProperty = async (source: {
        ...
        await focusNodeAndFitSub(
            source.mappedSuperClassId,
            nodeSubElementId(source.mappedSuperClassId, source.propertyId),
        );
    };
    const focusEmbeddableTypeProperty = async (source: {
        ...
        await focusNodeAndFitSub(
            source.embeddableTypeId,
            nodeSubElementId(source.embeddableTypeId, source.propertyId),
        );
    };
    ...
    return {
        ...
        copy: async (data?: LazyData<Partial<DeepReadonly<ModelGraphSubData>>>) => {
            try {
        ...
        paste: async (data?: LazyData<Partial<DeepReadonly<ModelGraphSubData>>>) => {
            try {
        ...
        cut: async (data?: LazyData<Partial<DeepReadonly<ModelGraphSubData>>>) => {
            try {
