...
export const tableToEntity = (
    ...
    for (const table of tables) {
        ...
        if (entity === undefined)
            throw new Error(`Table ${table.name} has no corresponding entity`);
    ...
    for (const table of tables) {
        ...
        const sourceTablePkColumns = sourceTable.columns.filter(
            (column) => column.partOfPrimaryKey,
        );
        const targetTablePkColumns = targetTable.columns.filter(
            (column) => column.partOfPrimaryKey,
        );
        ...
        if (
            ...
            sourceForeignKey.columnRefs.some(
                (it) => !sourceTablePkColumnNameSet.has(it.referencedColumnName),
            ) ||
            targetForeignKey.columnRefs.some(
                (it) => !targetTablePkColumnNameSet.has(it.referencedColumnName),
            )
        ) {
            notMidTables.push(table);
            continue;
        ...
        for (const index of table.indexes) {
            if (
                index.uniqueIndex &&
                index.columnNames.every((it) => sourceTablePkColumnNameSet.has(it))
            ) {
                sourceUnique = true;
            }
            if (
                index.uniqueIndex &&
                index.columnNames.every((it) => targetTablePkColumnNameSet.has(it))
            ) {
                targetUnique = true;
            }
        }
        ...
        for (const fk of table.foreignKeys) {
            for (const columnRef of fk.columnRefs) {
                ...
                if (fkList === undefined) {
                    ...
                    // fkList.push(fk)
                    throw new Error(
                        `Table ${table.name} ForeignKey ${fk.name} used column ${columnRef.columnName} already exists in other ForeignKey: (${fkList.map(
                            (it) => it.name,
                        )})`,
                    );
                }
        ...
        for (const column of table.columns) {
            ...
            // 外键
                ...
                if (fkList === undefined || fkList.length === 0) {
                    throw new Error(
                        `Table ${table.name} ForeignKey Column ${column.name} has no ForeignKey`,
                    );
                }
                for (const foreignKey of fkList) {
                    ...
                    if (referencedEntity === undefined)
                        throw new Error(
                            `Table ${foreignKey.referencedTableName} has no corresponding entity`,
                        );
                    ...
                    if (foreignKey.columnRefs.length === 0) {
                        ...
                        if (idEmbeddableType === undefined)
                            throw new Error(
                                `Table ${table.name} has no corresponding embeddable type`,
                            );
                        joinInfo = {
                            type: 'MultiColumn',
                    ...
                    const sourceColumns: DeepReadonly<Column>[] = foreignKey.columnRefs
                        .map((it) => {
                    ...
                    if (sourceColumns.length !== foreignKey.columnRefs.length) {
                        throw new Error(
                            `Table ${table.name}(${sourceColumns
                                .map((it) => it.name)
                                .join(', ')}) and ForeignKey ${
                                foreignKey.name
                            }(${foreignKey.columnRefs
                    ...
                    const lowerReferencedEntityName = nameTool.firstCaseToLower(
                    ...
                    if (indexList !== undefined) {
                        for (const index of indexList.filter((it) => it.uniqueIndex)) {
                            ...
                            const allSourceColumnInIndex = sourceColumns.every((sourceColumn) =>
                                indexColumnNameSet.has(sourceColumn.name),
                            );
                            if (allSourceColumnInIndex) {
                                ...
                                if (table.indexes.length === 1) {
                                    ...
                                    if ('key' in sourceProperty && 'keyGroups' in sourceProperty) {
                                        ...
                                        Object.assign(sourceProperty, {
                                            key: true,
                                            keyGroups: [index.name],
                                        });
                                    }
                                }
                            }
    ...
    const getIdEmbeddableType = (entity: EntityWithProperties): EmbeddableTypeWithProperties => {
        ...
        if (idEmbeddableType === undefined)
            throw new Error(
                `idEmbeddableType is undefined for entity: ${entity.name}(${entity.id})`,
            );
    ...
    for (const {
        ...
        const joinInfo: MidTableJoinInfo = {
            ...
            targetJoinInfo:
                referencedColumns.length === 1 && referencedColumns[0] !== undefined
                    ? {
                          type: 'SingleColumn',
                          foreignKeyType: 'REAL',
