...
<template>
    ...
    >
        ...
        <template #body>
            <div v-if="jvmType.serialized">@Serialized</div>
            <div
                class="viewer-item"
                v-if="jvmType.extraImports.length > 0"
            >
            ...
            <div
                class="viewer-item"
                v-if="jvmType.extraAnnotations.length > 0"
            >
            ...
            <div
                class="viewer-item"
                v-if="jvmType.sqlMatchRules.length > 0"
            >
                <span class="label no-drag">{{ translate('sqlMatchRules') }}</span>
                <ul>
                    <li
                        v-for="item in jvmType.sqlMatchRules"
                        class="match-rule"
                    >
            ...
            <div
                class="viewer-item"
                v-if="jvmType.tsMatchRules.length > 0"
            >
                <span class="label no-drag">{{ translate('tsMatchRules') }}</span>
                <ul>
                    <li
                        v-for="item in jvmType.tsMatchRules"
                        class="match-rule"
                    >
                        <span class="no-drag">{{ item.matchRegExp }}</span>
                    </li>
