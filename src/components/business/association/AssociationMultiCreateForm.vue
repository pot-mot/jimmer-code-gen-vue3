...
const sourceColumns = computed<GenTableModelInput_TargetOf_columns[]>(() =>
    association.value.columnReferences.map(it => it.sourceColumn).filter(it => it.name.length > 0)
)
...
const sourceTableOptions = computed<GenTableModelInput[]>(() =>
    MODEL.tables.filter(table => {
        ...
        return table.columns.some(column =>
            sourceColumns.value
                .map(it => getColumnCombineKey(it))
                .includes(getColumnCombineKey(column))
        )
    })
)
...
const sourceColumnOptions = computed<GenTableModelInput_TargetOf_columns[]>(() => {
    ...
    return [...columnCountMap.values()]
        .filter(it =>
            sourceTablesIsEmpty.value ? true : it.length === tables.length
        )
        .map(
            it => it[0]
        )
    ...
    <el-form style="width: calc(100% - 0.5rem);">
        <el-form-item :label="i18nStore.translate('LABEL_AssociationForm_mappingAssociation')">
            <table style="width: 100%;">
                <tr v-for="(columnReference, columnReferenceIndex) in association.columnReferences">
                    <td style="display: grid; grid-template-columns: 1fr 1fr 2em 1fr 1fr;">
                        ...
                        <el-select
                            :model-value="JSON.stringify(columnReference.sourceColumn)"
                            @change="(data: string) => handleSourceColumnChange(JSON.parse(data), columnReferenceIndex)"
                            clearable filterable
                            @clear="columnReference.sourceColumn = getDefaultColumn()"
                            ...
                            />
                            <template #label>
                                <el-text v-if="columnReference.sourceColumn.name">{{ columnReference.sourceColumn.name }}</el-text>
                                <el-text v-else type="info">{{ i18nStore.translate('LABEL_AssociationForm_sourceColumnName_placeholder') }}</el-text>
                            </template>
                        </el-select>
                        ...
                        <el-select
                            :model-value="JSON.stringify(columnReference.targetColumn)"
                            @change="(data: string) => handleTargetColumnChange(JSON.parse(data), columnReferenceIndex)"
                            clearable filterable
                            @clear="columnReference.targetColumn = getDefaultColumn()"
                            ...
                            />
                            <template #label>
                                <el-text v-if="columnReference.targetColumn.name">{{ columnReference.targetColumn.name }}</el-text>
                                <el-text v-else type="info">{{ i18nStore.translate('LABEL_AssociationForm_targetColumnName_placeholder') }}</el-text>
                            </template>
