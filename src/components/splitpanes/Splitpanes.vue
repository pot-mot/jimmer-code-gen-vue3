...
const emits = defineEmits<SplitpanesEmits>();
...
const getEventExtraData = (
    dataIndex: number | undefined = undefined,
): {
    index?: number;
    prevPane?: Pane;
    nextPane?: Pane;
...
const onSplitterClick = (event: MouseEvent | TouchEvent, splitterIndex: number) => {
    if ('ontouchstart' in window) {
        ...
        if (splitterTaps.value.splitter === splitterIndex) {
            ...
            splitterTaps.value.timeoutId = window.setTimeout(
                () => (splitterTaps.value.splitter = undefined),
                500,
            );
        }
    }
...
const getCurrentDragPercentage = (drag: DragInfo): number => {
    ...
    const containerSize =
        containerEl.value[props.horizontal ? 'clientHeight' : 'clientWidth'] * props.zoom;
...
const calculatePanesSize = (drag: DragInfo) => {
    ...
    const paneAfterMaxReached =
        paneAfter.max < 100 &&
        dragPercentage <= 100 - (paneAfter.max + sumNextPanesSize(splitterIndex + 1));
    ...
    if (paneBeforeMaxReached || paneAfterMaxReached) {
        if (paneBeforeMaxReached) {
            ...
            paneBefore.size = Math.max(
                100 - paneAfter.max - sums.prevPanesSize - sumNextPanesSize(splitterIndex + 1),
                0,
            );
...
const addSplitter = (paneIndex: number, nextPaneNode: HTMLElement, isVeryFirst = false) => {
    ...
    if (!isVeryFirst) {
        ...
        if (typeof window !== 'undefined' && 'ontouchstart' in window) {
            elm.addEventListener('touchstart', (event) => onMouseDown(event, splitterIndex), {
                passive: true,
            });
        }
...
const resetPaneSizes = (changedPanes: ChangedPanes = {}) => {
    ...
    else if (panes.value.some((pane) => pane.givenSize !== undefined || pane.min || pane.max < 100))
    ...
    if (leftToAllocate > 0.1) {
        for (const pane of panes.value) {
            if (pane.givenSize === undefined) {
                pane.size = Math.max(
                    Math.min(leftToAllocate / (panesCount.value - definedSizes), pane.max),
                    pane.min,
                );
            }
...
const render = () => {
    return h('div', {ref: containerEl, class: splitpanesClasses.value}, slots.default?.());
};

provide(injectKey.panes, panes);
provide(injectKey.indexedPanes, indexedPanes);
provide(
    injectKey.horizontal,
