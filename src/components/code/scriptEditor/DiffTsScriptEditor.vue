...
const validateAndCompile = async (): Promise<TsScriptValidatedCompileResult> => {
    const result = await props.executor.validateThenCompile(modifiedValue.value);
...
const executeCode = async (
    ...
    return await props.executor.executeTsArrowFunctionScript(modifiedValue.value, params);
};
