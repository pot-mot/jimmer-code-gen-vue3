...
type OneToOneMappedAbstractProperty = Omit<
...
type OneToManyAbstractProperty = Omit<
    OneToManyProperty,
    | 'category'
    | 'referencedEntityId'
    | 'name'
...
type ManyToManyMappedAbstractProperty = Omit<
...
type TransientProperty = {
    ...
    (
        | ({
              rawType: string;
          } & ({} | {resolver: string} | {resolverRef: string}))
        | ({
              referencedEntityId: string;
              typeIsList: boolean;
          } & ({resolver: string} | {resolverRef: string}))
...
type EmbeddableTypeProperty = ScalarCommonProperty | ScalarEnumProperty | ScalarEmbeddableProperty;
