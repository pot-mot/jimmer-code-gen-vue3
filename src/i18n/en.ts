...
export const localeEn: MainLocale = {
    ...
    script_declare_type_not_found: (scriptType: string) =>
        `Script type declaration not found: ${scriptType}`,
    script_can_contains_only_one_arrow_function_and_type_declares:
        'Script can only contain one arrow function and type declarations',
    parameter_count_mismatch: (expected: number, expectedTypes: string, actual: number) =>
        `Parameter count mismatch, expected ${expected} parameters (${expectedTypes}), actual ${actual}`,
    parameter_type_mismatch: (parameterName: string, expected: string, actual: string) =>
        `Parameter ${parameterName} type mismatch, expected ${expected}, actual ${actual}`,
    parameter_type_need_default_or_optional: (parameterName: string) =>
        `Parameter ${parameterName} type need default or optional`,
    parameter_type_cannot_optional: (parameterName: string) =>
        `Parameter ${parameterName} type cannot optional`,
    parameter_must_specify_type: (parameterName: string) =>
        `Parameter ${parameterName} must specify type`,
    arrow_function_cannot_get_signature: 'Arrow function cannot get signature',
    return_type_mismatch: (expected: string, actual: string) =>
