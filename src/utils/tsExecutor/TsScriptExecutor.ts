...
export const dangerousText = Object.freeze(['@ts-ignore', '@ts-nocheck']);

export const dangerousTypeTokens = Object.freeze(['any', 'unknown', 'never']);
...
// 使用 Proxy 拦截所有属性访问
const proxiedEnv = Object.freeze(
    new Proxy(
        {},
        {
...
export class TsScriptExecutor<Name extends ScriptTypeName> {
    ...
    constructor(scriptTypeName: Name) {
        this.scriptTypeName = scriptTypeName;
    }

    async validateThenCompile(code: string): Promise<TsScriptValidatedCompileResult> {
        ...
        try {
            if (!code) {
                return {
                    valid: false,
                    errorMessages: [
                        translate({key: 'not_blank_warning', args: [translate('code')]}),
                    ],
                };
            }
            ...
            const scriptTypeDeclareFile = program.getSourceFile(
                scriptTypeDeclares[scriptTypeName].fileName,
            );
            if (!scriptTypeDeclareFile) {
                errorMessages.push(
                    translate({key: 'script_declare_type_not_found', args: [scriptTypeName]}),
                );
                return {
                    valid: false,
                    errorMessages,
            ...
            if (
                scriptTypeDeclareFile.statements.length !== 1 ||
                !scriptTypeDeclareFile.statements[0]
            ) {
                errorMessages.push(
                    translate('script_can_contains_only_one_arrow_function_and_type_declares'),
                );
                return {
                    valid: false,
                    errorMessages,
            ...
            const stateMessage = translate(
                'script_can_contains_only_one_arrow_function_and_type_declares',
            );
            ...
            const declareParameters = scriptTypeSignature.getParameters().map((param) => {
                ...
                if (paramDeclarations && paramDeclarations.length === 1) {
                    ...
                    if (
                        ...
                        'type' in declaration &&
                        declaration.type &&
                        typeof declaration.type === 'object' &&
                        'getFullText' in declaration.type &&
                        typeof declaration.type.getFullText === 'function'
                    ) {
                ...
                // 检查参数是否可选
                const isOptional =
                    paramType.getFlags() & ts.TypeFlags.Union
                        ? (paramType as ts.UnionType).types.some(
            ...
            if (actualParameters.length !== declareParameters.length) {
                ...
                const message = translate({
                    ...
                    args: [
                        declareParameters.length,
                        paramTypesLiteral.join(', '),
            ...
            for (let i = 0; i < actualParameters.length; i++) {
                ...
                const actualParamType = actualParam.type
                    ? typeChecker.getTypeAtLocation(actualParam.type)
                    : undefined;
                const isTypeOptional =
                    actualParamType && actualParamType.getFlags() & ts.TypeFlags.Union
                        ? (actualParamType as ts.UnionType).types.some(
                              (t) => t.getFlags() & ts.TypeFlags.Undefined,
                          )
                        : false;
            ...
            if (!arrowTypeSignature) {
                markers.push(
                    createMarker(arrowFunction, translate('arrow_function_cannot_get_signature')),
                );
            } else {
                ...
                if (!typeChecker.isTypeAssignableTo(actualReturnType, scriptReturnType)) {
                    ...
                    markers.push(
                        createMarker(
                            arrowFunction.type ?? arrowFunction,
                            translate({
